...
#         self.dl = cycle(dataloader['dataloader'])
    ...
    def load_pretrained(self, pretrain_path, verbose=False):
        """Load pretrained weights from an arbitrary checkpoint for few-shot adaptation.
        Only loads model & EMA weights; optimizer is kept fresh for fine-tuning.
        """
        if self.logger is not None and verbose:
            self.logger.log_info(f'Loading pretrained weights from {pretrain_path}')
        data = torch.load(pretrain_path, map_location=self.device)
        self.model.load_state_dict(data['model'])
        self.ema.load_state_dict(data['ema'])
        # Intentionally NOT loading optimizer — use fresh lr for fine-tuning
        # Reset counters so training starts from epoch 0
        self.step = 0
        self.milestone = 0
        self.current_epoch = 0
    ...
    def train(self, start_epoch=None):
        ...
        with tqdm(initial=0, total=total_steps) as pbar:
            # 续跑时可能从 e>2 开始，本进程中从未执行过 e==1 分支，
            # 因此这里给 fixed_weights 一个合理的默认初始化，避免未定义错误。
            fixed_weights = {i: 0.97 for i in range(num_datasets)}
                    ...
                    if sum(batch_cnt) >= self.train_batches:
                        with torch.no_grad():
                            ...
                            if self.milestone % 1 == 0:
                                self.save(self.milestone, epoch=e+1)  # 保存下一个 epoch 的编号
                        break
