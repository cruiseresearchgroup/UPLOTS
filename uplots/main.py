...
def parse_args():
    ...
    # args for few-shot / zero-shot
    parser.add_argument('--pretrain_path', type=str, default=None,
                        help='Path to pretrained checkpoint for few-shot adaptation')
...
#     print('Avalable instruct_list: ',results) 
    ...
    ett_ins = {
        ...
        # high-load / low-load (amplitude-based)
        ...
        # few-shot (reuse same prompt as full version)
        'fewshot100_evening_peak_etth':'ETTEP', 'fewshot300_evening_peak_etth':'ETTEP', 'fewshot500_evening_peak_etth':'ETTEP',
        'fewshot100_morning_peak_energy':'ENEMP', 'fewshot300_morning_peak_energy':'ENEMP', 'fewshot500_morning_peak_energy':'ENEMP',
        # legacy keys
    ...
    if args.train:
        if args.pretrain_path:
            # ---- Few-shot adaptation: load pretrained weights, train from epoch 0 ----
            trainer.load_pretrained(args.pretrain_path, verbose=True)
            if logger is not None:
                logger.log_info(f'Few-shot mode: loaded pretrained weights from {args.pretrain_path}')
            trainer.train(start_epoch=0)
        else:
            # ---- Normal training: auto-detect or load specified checkpoint ----
