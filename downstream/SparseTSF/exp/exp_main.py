...
class Exp_Main(Exp_Basic):
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                dec_inp = torch.zeros_like(batch_y[:, -self.args.pred_len:, :]).float()
                dec_inp = torch.cat([batch_y[:, :self.args.label_len, :], dec_inp], dim=1).float().to(self.device)
                # print('batch_x shape', batch_x.shape)#  [32, 336, 321]
                # print('batch_x_mark shape', batch_x_mark.shape)  # [32, 336, 4] 
                # print('dec_inp shape', dec_inp.shape) # [32, 768, 321]
                # print('batch_y_mark shape', batch_y_mark.shape) # [32, 768, 4]
                ...
                if self.args.lradj == 'TST':
                    ...
                    scheduler.step()
                # early_stopping(0, self.model, path)
    ...
    def test(self, setting, test=0):
        ...
        print("Test cost time: {}".format(time.time() - test_time))
        return
