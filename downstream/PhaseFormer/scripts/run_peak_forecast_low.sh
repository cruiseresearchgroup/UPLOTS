#!/bin/bash
# PhaseFormer peak forecasting: low-data regime (1%, 3%, 5%, 7%) orig + 10%+Gen already done
set -e
...
for ds in "${DATASETS[@]}"; do
    ...
    for R in 0.01 0.03 0.05 0.07; do
        RPCT=$(echo "$R" | awk '{printf "%d", $1*100}')
        run_exp "PhaseFormer_${DS_NAME}_r${RPCT}_aug" "$ORI_FILE" "$R" "$AUG_FILE"
    done
done

echo "=== All PhaseFormer low-data experiments done ==="
